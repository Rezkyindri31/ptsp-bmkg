...
import useTampilanInformasiGeofisika from "@/hooks/Backend/useTampilanInformasiGeofisika";
...
export default function InformasiGeofisika() {
  const { produkInformasiGeofisika, loading, error } =
    useTampilanInformasiGeofisika();
  const {
    itemTerkini,
    activePage,
    totalHalaman,
    ...
    prevPage,
    getItemProps,
  } = usePagination(produkInformasiGeofisika);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {itemTerkini.map((product) => (
          <Popover key={product.id}>
            <PopoverHandler>
              <Card className="border-2 hover:shadow-2xl transition relative py-6 shadow-lg ">
